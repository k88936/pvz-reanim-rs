...
fn generate_makefile(
    ...
    let webp_output_rel = make_relative(makefile_dir, webp_output);
    let reanim_dir_esc = reanim_dir_rel.to_string_lossy();
    let webp_output_esc = webp_output_rel.to_string_lossy();
    ...
    if files.is_empty() {
        ...
        content.push_str("all:");
        for info in files {
            content.push_str(&format!(" $(WEBP_OUTPUT)/{}/.keep", info.stem));
        }
        content.push_str("\n");
        content.push_str("\n");
